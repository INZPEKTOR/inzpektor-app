...
export class NoirService {
  ...
  private async initWasm() {
    ...
    try {
      // Initialize ACVM WASM
      await initACVM(new URL('/wasm/acvm_js_bg.wasm', import.meta.url).toString());
      console.log('[NoirService] ✓ ACVM WASM initialized');

      // Initialize NoirC ABI WASM
      await initNoirC(new URL('/wasm/noirc_abi_wasm_bg.wasm', import.meta.url).toString());
      console.log('[NoirService] ✓ NoirC ABI WASM initialized');

      this.initialized = true;
    } catch (error) {
      console.error('[NoirService] ❌ Failed to initialize WASM:', error);
      throw error;
    }
  }
  /**
